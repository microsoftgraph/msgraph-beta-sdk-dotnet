    ...
    public class Given_a_valid_SharePoint_Site : GraphTestBase
    {
        ...
        public async Async.Task It_gets_the_sites_drives()
        {
            try
            {
                ...
                Assert.True(drives.Count > 0, "Expected at least one drive result. Got zero. Check test data.");
        ...
        public async Async.Task It_gets_a_site_by_path()
        {
            try
            {
                ...
                Assert.Contains(portalInfoTechSite.WebUrl, siteResource);
                Assert.Contains(portalInfoTechSite.Id, portalInfoTechSite.SiteCollection.Hostname); // Check if id format changes under us. 
