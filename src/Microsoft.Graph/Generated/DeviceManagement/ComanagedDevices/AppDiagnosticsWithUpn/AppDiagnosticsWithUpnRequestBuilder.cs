...
namespace Microsoft.Graph.Beta.DeviceManagement.ComanagedDevices.AppDiagnosticsWithUpn {
    ...
    public class AppDiagnosticsWithUpnRequestBuilder {
        ...
        public AppDiagnosticsWithUpnRequestBuilder(Dictionary<string, object> pathParameters, IRequestAdapter requestAdapter, string upn = "") {
            _ = pathParameters ?? throw new ArgumentNullException(nameof(pathParameters));
            _ = requestAdapter ?? throw new ArgumentNullException(nameof(requestAdapter));
            UrlTemplate = "{+baseurl}/deviceManagement/comanagedDevices/appDiagnostics(upn='{upn}'){?%24top,%24skip,%24search,%24filter,%24count}";
            var urlTplParams = new Dictionary<string, object>(pathParameters);
            if (!string.IsNullOrWhiteSpace(upn)) urlTplParams.Add("upn", upn);
