...
namespace Microsoft.Graph.Beta.Drive.SearchWithQ {
    ...
    public class SearchWithQRequestBuilder {
        ...
        /// </summary>
        public async Task<SearchWithQResponse> GetAsync(Action<IDictionary<string, string>> headers = default, IEnumerable<IRequestOption> options = default, IResponseHandler responseHandler = default, CancellationToken cancellationToken = default) {
            var requestInfo = CreateGetRequestInformation(headers, options);
            return await RequestAdapter.SendAsync<SearchWithQResponse>(requestInfo, SearchWithQResponse.CreateFromDiscriminatorValue, responseHandler, default, cancellationToken);
        }
    }
}
